...
- [x] **ログインに３回以上失敗**するとエラー表示させログインできないようにします<br><br>
- [x] **CSRF対策**としてデータ登録のページには**ワンタイムトークン**を使用してエラー表示する<br><br>
- [x] **XSS対策**としてすべて入力したものは**htmlspecialchars関数**を使いサニタイジング化<br><br>
- [x] **入力フィルタ**では**正規表現**でデータ（電話番号、メールアドレスなど）を確認<br><br>
- [x] 入力データ（サービス登録時）に「http」を含めるとエラー表示<br><br>
- [x] パスワードは暗号化して登録
...
- [x] DNSサーバーのドメイン登録の際は、プライバシーの保護を有効化<br><br>
